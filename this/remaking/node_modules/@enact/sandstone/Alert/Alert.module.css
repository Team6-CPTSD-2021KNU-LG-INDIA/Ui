...
:global(.enact-locale-right-to-left) .alert.overlay .buttonContainer {
  margin-left: 0;
...
:global(.enact-locale-right-to-left) .alert.overlay .content {
  margin-left: 0.75rem;
  margin-right: 1rem;
}
:global(.enact-locale-ja) .alert.overlay .content,
...
.popup:global(.light).overlay {
  background-color: rgba(202, 203, 204, 0.95);
  box-shadow: 0 0.75rem 0.75rem rgba(46, 50, 57, 0.3);
}
